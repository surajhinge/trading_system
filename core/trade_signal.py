...
def _has_recent_rejection(candles, price, direction, lookback=6):
    """Check whether a recent 15M candle rejected the important level."""
...
def _base_no_trade(context, location, reason, **kwargs):
    """Create a detailed WAIT/NO_TRADE response."""
...
def evaluate(
    ...
    `nifty_daily_candles` is optional for backward compatibility.

    When supplied, NIFTY contributes to:
    ...
    if location.zone in ("near_untested_low", "near_untested_high"):
        return _base_no_trade(
            ...
            location,
            (
                f"Level only touched "
                f"{location.support_touches if 'low' in location.zone else location.resistance_touches} "
                f"time(s) - not a proven level"
            ),
            nifty_regime=market.regime,
        )
    ...
    if location.zone not in ("near_support", "near_resistance"):
        return _base_no_trade(
            context,
            location,
            f"1H zone is {location.zone} - no setup evaluated",
            nifty_regime=market.regime,
        )
    ...
    # ---------------------------------------------------------
    # 6. DAILY TREND FILTER
    # ---------------------------------------------------------
    if direction == "long" and context == "Bearish":
        return _base_no_trade(
            context,
            location,
            "Daily context is Bearish - counter-trend long skipped",
            nifty_regime=market.regime,
        )
    ...
    if direction == "short" and context == "Bullish":
        return _base_no_trade(
            context,
            location,
            "Daily context is Bullish - counter-trend short skipped",
            nifty_regime=market.regime,
        )

    # ---------------------------------------------------------
    # 7. IMPORTANT 1H LEVEL
    # ---------------------------------------------------------
    level = (
        location.nearest_support
        if direction == "long"
        else location.nearest_resistance
    )

    # ---------------------------------------------------------
    # 8. 15M REJECTION
    # ---------------------------------------------------------
    rejection = _has_recent_rejection(
        fifteen_min_candles,
        level,
        direction,
    )
    ...
    if not rejection:
        return _base_no_trade(
            ...
            (
                ...
                f"but no recent 15M rejection"
            ),
            nifty_regime=market.regime,
        )

    # ---------------------------------------------------------
    # 9. 15M STRUCTURE + TRIGGER
    # ---------------------------------------------------------
    confirmation = (
        confirm_long_entry(
            fifteen_min_candles,
    ...
    # Structural invalidation must be on the correct side
    # of the entry.
    if direction == "long" and stop >= entry:
        return _base_no_trade(
            context,
            location,
            "Structural stop is not below the long entry",
            rejection_confirmed=True,
            structure_confirmed=True,
            trigger_confirmed=True,
            trigger_price=confirmation.trigger_price,
    ...
    if direction == "short" and stop <= entry:
        return _base_no_trade(
            context,
            location,
            "Structural stop is not above the short entry",
            rejection_confirmed=True,
            structure_confirmed=True,
            trigger_confirmed=True,
            trigger_price=confirmation.trigger_price,
    ...
    fib = compute_fib_zone(
        location.nearest_support,
        location.nearest_resistance,
        current_price,
        direction,
    )

    # ---------------------------------------------------------
    # 12. NEXT MARKET TARGET
    # ---------------------------------------------------------
    ...
    if target is None:
        return _base_no_trade(
            context,
            location,
            "No confirmed 1H swing available as target",
            rejection_confirmed=True,
            structure_confirmed=True,
            trigger_confirmed=True,
            trigger_price=confirmation.trigger_price,
    ...
    if not plan.meets_minimum_rr:
        return _base_no_trade(
            ...
            location,
            (
                f"Actual target gives R:R "
                f"{plan.risk_reward_ratio:.2f}, "
                f"below minimum {MIN_RR:.2f}"
            ),
            rejection_confirmed=True,
            structure_confirmed=True,
            trigger_confirmed=True,
            ...
            actual_risk=round(
                size.quantity * size.risk_per_share,
                2,
            ),
            volume_confirmed=volume.confirmed,
            volume_ratio=volume.volume_ratio,
    ...
    if score.score < 70:
        return _base_no_trade(
            ...
            location,
            (
                f"Setup passed structure and R:R, "
                f"but score {score.score}/100 is below 70"
            ),
            rejection_confirmed=True,
            structure_confirmed=True,
            trigger_confirmed=True,
    ...
    # ---------------------------------------------------------
    # 17. NIFTY MARKET REGIME FILTER
    # ---------------------------------------------------------
    if direction == "long" and market.regime == "Bearish":
        return _base_no_trade(
            context,
            location,
            "NIFTY regime is Bearish - long not supported",
            rejection_confirmed=True,
            structure_confirmed=True,
            trigger_confirmed=True,
            trigger_price=confirmation.trigger_price,
    ...
    if direction == "short" and market.regime == "Bullish":
        return _base_no_trade(
            context,
            location,
            "NIFTY regime is Bullish - short not supported",
            rejection_confirmed=True,
            structure_confirmed=True,
            trigger_confirmed=True,
            trigger_price=confirmation.trigger_price,
    ...
    reason = (
        f"{direction.upper()} confirmed | "
        f"rejection + structure + trigger | "
        f"target ₹{target:.2f} | "
        f"R:R {plan.risk_reward_ratio:.2f} | "
    ...
    return TradeSignal(
        ...
        fibonacci_confirmed=fib.in_golden_zone,
        fibonacci_retracement_pct=(
            fib.current_retracement_pct * 100
        ),
